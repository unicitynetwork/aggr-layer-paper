...
\begin{figure}[!htbp]
    \begin{center}
        ...
        \end{tikzpicture}
        %\includegraphics[width=.4\textwidth]{pic/Layers}
...
% Let there be 31 validators in the BFT Consensus Layer; up to 10 of those may be faulty. Each PoW validator includes a BFT validator identity as its coinbase address (beneficiary of the eventual block rewards). PoW chain is divided to epochs, e.g., 10~000 blocks. Soon after the end of each epoch, 7 of the worst performing (by number of failed rounds as the leader), or if equal, then oldest serving Consensus Layer validators are replaced: new ones are identified by the top 7 of the unique PoW coinbase addresses.
...
% todo: graph comparing hash-based proof with different zk tech; which parameters make sense etc.

% todo: obtaining full trust - so that users can validate the correct operation of instrastructure

% todo: comparison with nullifier based setups (zerocash, aztec, tornado cash)
...
%\bibitem{BaHaSt93}
% Bayer, D., Haber, S., Stornetta, W.-S.: Improving the efficiency and reliability of digital timestamping. In: Sequences
% II: Methods in Communication, Security, and Computer Sci., pp. 329--334. Springer, Heidelberg (1993)

% \bibitem{BuLa13} Buldas, A., Laanoja, R.: Security proofs for hash tree time-stamping using hash functions with small ou
% tput size. In: Boyd, C., Simpson, L. (Eds.): ACISP 2013, LNCS 7959, pp. 235--250, 2013. Springer, Heidelberg (2013)
