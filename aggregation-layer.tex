...
Here we assume a model where users have the ability to validate all aspects of the system operation, relevant to particular user's assets. The trustlessness level is analogous to the highest standards of trustlessness introduced by Bitcoin~\cite{bitcoin} where each ``client'' is actually a full validator of the system, and starts with downloading and validating the blockchain from the genesis block.

The Root of Trust is PoW blockchain. Thus, a maximalist user maintains a full node of the PoW chain, and when he receives a token then the user must be able to efficiently verify if: 1) Token is valid (elaborated elsewhere), 2) the Aggregation layer is not forked, 3) Aggregation layer did not certify alternative states of his token.

The 2nd point is achieved by validating the unique state root snapshot in PoW block header. It takes some time before the cumulative state snapshot appears and the block can be considered final, thus maximalist verification is not available immediately.

The 3rd point is achieved by validating the correct operation of the Aggregation Layer, specifically if there were no Unicity Proofs generated for the token which are not recorded in the token history. To do so, all the non-deletion proofs from the token genesis to current state must be validated. This is made efficient by recursive ZK proof, showing that each round's non-deletion proof is valid and there were no rounds in between. Such recursive proof is generated periodically and made available with some delay.

\subsection{Practical Security Assumptions}

If we relax the model by trusting that a majority of BFT Consensus Nodes do not co-operate in malicious way to allow Aggregation Layer nodes to produce false proofs, it is possible to offer very practical system operation parameters. Inclusion Proof is returned within a few seconds, thus a transaction can be finalized and verified by anyone, without consulting any external data, within seconds.

The Root of Trust is the set of BFT layer's epoch change records, which grows very slowly (e.g., once aggregated signature per day or week). Once switching to PoS (Section~\ref{sec:consensus-roadmap}), the Root of Trust remains the same.
...
The Root of Trust is a PoW blockchain. A maximalist user maintains a full node of this chain. Upon receiving a token, the user must be able to efficiently verify the following:

\begin{enumerate}
    \item The token is valid (as elaborated elsewhere),
    \item The Aggregation Layer has not forked,
    \item The Aggregation Layer has not certified conflicting states for the same token.
\end{enumerate}
...
The third point is addressed by verifying the correct operation of the Aggregation Layer—specifically, ensuring that no Inclusion Proofs have been generated for the token that are not reflected in its recorded history. To achieve this, all non-deletion proofs from the token's genesis up to its current state must be validated. This is made efficient through the use of recursive zero-knowledge proofs (ZKPs), which demonstrate that each round’s non-deletion proof is valid and that no rounds were skipped from verification. These recursive proofs are generated periodically and are made available with considerable latency.

\subsection{Practical Security Assumptions}

If we relax the model by assuming that a majority of BFT consensus nodes do not collude maliciously with Aggregation Layer nodes to produce false proofs, the system can achieve significantly more practical operational parameters.

In this scenario, an inclusion proof can be returned within a few seconds, allowing a transaction to be finalized and independently verified—without consulting external data—within the same timeframe.

Here, the Root of Trust is defined by the set of epoch change records in the BFT consensus layer. These records grow very slowly (e.g., one aggregated signature per day or week). When transitioning to proof-of-stake (PoS) consensus (see Section~\ref{sec:consensus-roadmap}), the Root of Trust remains the same.
