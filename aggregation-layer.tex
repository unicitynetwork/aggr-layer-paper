  ...
  \begin{algorithmic}[1]
    ...
    \Function{ComputeForest}{$\pi, p$}
      \For{$\ell \gets \text{tree\_depth}-1 \textbf{ to } 0$}
        ...
        \While{$i < |p|$}
          \State $(k, v) \gets p[i]$
          \State $i_p \gets \lfloor k / 2 \rfloor$ \Comment{parent index}
          \State $\text{is\_right} \gets k \bmod 2$
          \State $i_s \gets 2i_p + (1 - \text{is\_right})$ \Comment{sibling idx}
          \If{$\lnot \text{is\_right} \land \allowbreak |p| > i+1 \land p[i+1].k = i_s$}
            \State        \Comment{Special case: }
            \State $i \gets i + 1$ \Comment{right sibling is next}
            \State $v_s \gets p[i].v$ \Comment{Sibling value}
          \Else
