...
We have implemented the non-deletion proof verification algorithm as a Rust program~\cite{stark}. As a commitment to the ``right'' program we use a prover key, generated during program setup. Its contents are: a commitment to the preprocessed traces, the starting Program Counter register, the starting global digest of the program, after incorporating the initial memory; the chip information, the chip ordering; and prover configuration.
...
\bibitem{snark} Trustless SMT accumulator. Github repository. \url{https://github.com/unicitynetwork/nd-smt}

\bibitem{stark} SP1 zkVM based consistency proof. Github repository. \url{https://github.com/unicitynetwork/zkvm-ndsmt}

\bibitem{randomx} Tevador: RandomX: Experimental proof of work algorithm based on random code execution. Github repository. \url{https://github.com/tevador/RandomX}
